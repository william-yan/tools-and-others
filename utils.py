...
class DataLoader():
    ...
    def __init__(self,f_prefix, batch_size=5, seq_length=20, num_of_validation = 0, forcePreProcess=False, infer=False, generate = False):
        ...
        # 设置文件维度
        self.dataset_dimensions = {'biwi':[720, 576], 'mot':[768, 576]}
        ...
        # 检查验证数据集的可用性，如果它大于可用的验证数据集，则剪切请求的数字
        ...
        # 用于创建dict键：文件夹名称，值：此文件夹中的文件
        self.create_folder_file_dict()

        '''
        在工程中，文件已经建立，所以这些数据处理函数并没有使用，主要的是使用了数据导入函数
        '''
        # 数据处理函数，如果需要验证数据，且验证数据存在或者强制处理数据
        if self.additional_validation:
    ...
    def load_preprocessed(self, data_file, validation_set = False):
        '''
        数据导入函数，
             导入data_file中处理好的数据，并将数据返回
             数据：/data/train/trajectories_train.cpkl
        '''
